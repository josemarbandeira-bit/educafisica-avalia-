        ...
        <form method="post" id="examCreateForm">
          ...
          <div class="card">
            <div class="section-heading compact-heading">
              <div>
                ...
                <h2>Selecione um ou mais assuntos</h2>
                <p>Você pode misturar conteúdos na mesma prova. O número ao lado mostra quantas questões existem para a série escolhida.</p>
              </div>
            ...
            <div class="topic-multi-status" id="topicMultiStatus">
              Nenhum assunto marcado = o sistema poderá usar todos os assuntos disponíveis.
            </div>
          ...
          <div class="card" id="autoBox">
            ...
            <div class="info-box v18-info-box">
              <b>Como será a prova:</b> o mesmo link pode ser enviado para toda a turma.
              Cada aluno recebe uma combinação individual de questões e alternativas.
              Se você selecionar mais de um assunto, todos entram no banco de sorteio.
            </div>
        ...
        function updateTopicStatus(availableCount = null) {
          ...
          if (!selected.length) {
            topicMultiStatus.innerHTML =
              '<b>Nenhum assunto marcado.</b> O gerador poderá usar todos os ' +
              visible + ' assuntos disponíveis para esta série.';
          } else {
            const total = selected.reduce((sum, topic) => sum + currentTopicCount(topic), 0);

            topicMultiStatus.innerHTML =
              '<b>' + selected.length + ' assunto(s) selecionado(s)</b> · ' +
              total + ' questão(ões) no banco compatível.';
          }
        }
...
async function createExam(request, env) {
  ...
  if (mode === 'manual') {
    ...
    const topics = f.getAll('topic_filters')
      .map(x => String(x || '').trim())
      .filter(Boolean);
    const difficulty = String(f.get('difficulty_filter') || '').trim();
    const count = Math.max(1, Math.min(100, Number(f.get('question_count') || 10)));
    ...
    // Mantém um pool maior do que a quantidade exibida para o aluno.
    // Isso aumenta bastante a variação entre aparelhos.
    const poolLimit = Math.max(
      count,
      Math.min(500, topics.length ? topics.length * 100 : Math.max(200, count * 4))
    );
    ...
    const picked = await env.DB.prepare(sql)
      .bind(...binds, poolLimit)
      .all();
    ...
    if (ids.length < count) {
      const criteria = [
        ...
        topics.length ? topics.join(' + ') : 'todos os assuntos',
      ...
      return page(
        ...
        `
          <div class="login">
            ...
            <p>Escolha mais assuntos, uma dificuldade mista ou importe o Banco Mestre.</p>
...
function css() {
  return `<style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;font-family:Inter,Arial,Helvetica,sans-serif;background:var(--bg);color:var(--ink);-webkit-font-smoothing:antialiased}
    ...
    @media(max-width:900px){.topic-multi-grid{grid-template-columns:1fr 1fr}.question-image-editor-grid{grid-template-columns:1fr}.dash-primary-actions{grid-template-columns:1fr 1fr}.dash-trimester-grid{grid-template-columns:1fr}.dash-notes-top{grid-template-columns:1fr}.dash-average-grid{grid-template-columns:1fr}.dash-average-panel.wide{grid-column:auto}.dash-mean-list.scroll{grid-template-columns:1fr}.dash-hero{align-items:flex-start;flex-direction:column}.rco-help-strip{grid-template-columns:1fr 1fr}.rco-hero{align-items:flex-start;flex-direction:column}.rco-toolbar{align-items:flex-start;flex-direction:column}.rco-search{min-width:0;width:100%}.dashboard-means{grid-template-columns:1fr}.share-exam-card{grid-template-columns:1fr}.qr-box{max-width:300px}.quick-actions{grid-template-columns:1fr 1fr}.navlinks a span{display:none}.navlinks a{font-size:18px;padding:9px}.navlinks .logout-link{font-size:12px}.welcome-mark{width:105px;height:105px}.exam-grid{grid-template-columns:1fr}}
