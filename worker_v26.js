      ...
      <main class="dashboard-v16 dashboard-v21">
        ...
        <section class="future-home-hero">
          ...
          <div class="future-hero-copy">
            ...
            <h1>Professor no controle.<br><em>Dados que ajudam a decidir.</em></h1>
            <p>
              Crie avaliações, acompanhe turmas e visualize resultados com clareza,
              sem poluição na tela.
            </p>
          ...
          <div class="future-command-card">
            ...
            <div class="future-signal">
              <span><i></i></span>
              <div><b>Sistema operacional</b><small>Banco e avaliações conectados</small></div>
            </div>
          </div>
        </section>

        <section class="dash-primary-actions v21-primary-actions v26-primary-actions">
          <a href="/provas/nova"><span>＋</span><div><b>Criar prova</b><small>Nova avaliação</small></div></a>
          <a href="/alunos" class="v26-featured-action"><span>◎</span><div><b>Alunos e resultados</b><small>Filtrar escola e turmas</small></div></a>
          <a href="/caderno"><span>▦</span><div><b>Caderno RCO</b><small>Copiar e baixar notas</small></div></a>
          <a href="/banco"><span>▤</span><div><b>Questões</b><small>Banco e imagens</small></div></a>
          <a href="/resultados"><span>▥</span><div><b>Provas</b><small>Resultados por avaliação</small></div></a>
        </section>
...
function studentStartPage(e, error = '') {
  ...
  return `
    <main class="student">
      ...
      <section class="card exam-rules-card">
        ...
        <div>
          ...
          <div class="exit-penalty-rule">
            <div class="exit-penalty-rule-icon">⚠️</div>
            <div>
              <b>Sair da tela desconta pontos da nota.</b>
              <p>
                Nesta prova, <strong>cada saída registrada desconta ${penaltyPerExitText} ponto(s)</strong>.
                O cálculo é: <strong>valor da prova ÷ 50 minutos</strong>.
                Trocar de aplicativo, minimizar ou sair da página pode ser registrado como saída.
              </p>
            </div>
          </div>
      ...
      <form method="post" class="card start-exam-form">
        ...
        <label class="ack-row">
          ...
          <span>Li as orientações. Sei que o tempo não será pausado e que cada saída registrada da tela descontará pontos da minha nota.</span>
        </label>
    ...
    <style>
      ...
      .exit-penalty-rule{margin-top:12px;padding:13px;border-radius:13px;background:#fff0f0;border:1px solid #efc2c6;color:#8b3038;display:flex;gap:10px;align-items:flex-start}
      .exit-penalty-rule-icon{font-size:25px;line-height:1}
      .exit-penalty-rule b{font-size:14px}
      .exit-penalty-rule p{margin:4px 0 0;font-size:12px;line-height:1.5}
      .exit-penalty-rule strong{color:#b02e38}
      ...
      @media(max-width:650px){.legacy-recalc-summary,.legacy-recalc-result{grid-template-columns:1fr 1fr}.legacy-recalc-card{padding:17px}.legacy-recalc-card h1{font-size:26px}.v22-students-title{align-items:flex-start;flex-direction:column}.rco-export-actions{width:100%}.rco-export-actions .btn{flex:1}.v22-student-card{grid-template-columns:34px 1fr 56px}.class-performance-card>summary{align-items:flex-start;flex-direction:column}.class-performance-summary{width:100%;justify-content:space-between}.class-performance-summary>span{text-align:left}.class-exam-head{align-items:flex-start;flex-direction:column}.v21-trimester-grid{grid-template-columns:1fr!important}.batch-class-head{align-items:flex-start;flex-direction:column}.batch-class-grid{grid-template-columns:repeat(3,1fr)}.batch-created-share{grid-template-columns:1fr}.batch-created-qr{max-width:220px;margin:auto}.batch-created-top{grid-template-columns:34px 1fr}.batch-created-ok{grid-column:1/-1;width:max-content}.batch-result-footer{flex-direction:column}.batch-result-footer .btn{width:100%}.topic-multi-grid{grid-template-columns:1fr}.topic-multi-tools{width:100%}.topic-multi-tools .btn{flex:1}.manual-question-thumb{width:72px;height:54px}.bank-thumb{width:64px;height:46px}.exam-rules-card{grid-template-columns:1fr}.rule-list{grid-template-columns:1fr}.rules-icon{width:52px;height:52px}}
    </style>
  `;
}
...
function studentForm(e, qs, attempt, savedMap = {}) {
  ...
  return `
    <main class="student exam-secure" id="secureExam">
      ...
      <div class="exit-penalty-live">
        <div>
          <span>⚠️</span>
          <div>
            <b>Não saia da tela da prova.</b>
            <small>Cada saída registrada desconta ${penaltyPerExitText} ponto(s).</small>
          </div>
        </div>
        <strong id="exitPenaltyLive">
          ${Number(attempt.suspicious_events || 0)} saída(s) · -${currentPenaltyText} ponto(s)
        </strong>
      </div>
      ...
      <div class="focus-warning" id="focusWarning">
        <div>
          ...
          <p id="focusWarningText">O relógio continuou contando e a saída gera desconto na nota. Volte à avaliação.</p>
    ...
    <style>
      ...
      .exit-penalty-live{position:relative;z-index:2;margin:0 0 9px;padding:10px 12px;border-radius:12px;background:#fff0f0;border:1px solid #efc2c6;color:#8b3038;display:flex;align-items:center;justify-content:space-between;gap:10px}
      .exit-penalty-live>div{display:flex;align-items:center;gap:8px}
      .exit-penalty-live>div>span{font-size:22px}
      .exit-penalty-live>div>div{display:flex;flex-direction:column}
      .exit-penalty-live b{font-size:12px}
      .exit-penalty-live small{font-size:9px;color:#9a4a51}
      .exit-penalty-live>strong{font-size:11px;white-space:nowrap;background:white;border:1px solid #efcfd2;border-radius:999px;padding:6px 8px}
...
function nav() {
  return `<header class="topbar">
    ...
    <nav class="navlinks">
      ...
      <a href="/alunos">◎ <span>Alunos</span></a>
...
.result-final-note{margin:18px 0 0;color:var(--green);font-weight:800}
    ...
    @media(max-width:900px){.rco-export-panel{align-items:flex-start;flex-direction:column}.rco-export-actions{justify-content:flex-start}.v22-student-grid{grid-template-columns:1fr}.v21-notes-head{align-items:flex-start;flex-direction:column}.v21-rco-button{align-items:flex-start}.topic-performance-grid{grid-template-columns:1fr}.class-performance-summary{gap:10px}.class-exam-metrics{grid-template-columns:1fr}.batch-class-grid{grid-template-columns:repeat(5,1fr)}.batch-created-grid{grid-template-columns:1fr}.batch-copy-all{grid-template-columns:1fr}.batch-result-hero{align-items:flex-start;flex-direction:column}.topic-multi-grid{grid-template-columns:1fr 1fr}.question-image-editor-grid{grid-template-columns:1fr}.dash-primary-actions{grid-template-columns:1fr 1fr}.dash-trimester-grid{grid-template-columns:1fr}.dash-notes-top{grid-template-columns:1fr}.dash-average-grid{grid-template-columns:1fr}.dash-average-panel.wide{grid-column:auto}.dash-mean-list.scroll{grid-template-columns:1fr}.dash-hero{align-items:flex-start;flex-direction:column}.rco-help-strip{grid-template-columns:1fr 1fr}.rco-hero{align-items:flex-start;flex-direction:column}.rco-toolbar{align-items:flex-start;flex-direction:column}.rco-search{min-width:0;width:100%}.dashboard-means{grid-template-columns:1fr}.share-exam-card{grid-template-columns:1fr}.qr-box{max-width:300px}.quick-actions{grid-template-columns:1fr 1fr}.navlinks a span{display:none}.navlinks a{font-size:18px;padding:9px}.navlinks .logout-link{font-size:12px}.welcome-mark{width:105px;height:105px}.exam-grid{grid-template-columns:1fr}}
