...
async function dashboard(env) {
  ...
  for (const e of exams.results) {
    ...
    if (!schoolGroup.shifts.has(shift)) {
      schoolGroup.shifts.set(shift, new Map());
    }

    const shiftGroup = schoolGroup.shifts.get(shift);

    if (!shiftGroup.has(className)) {
      shiftGroup.set(className, []);
    }

    shiftGroup.get(className).push(e);
  }

  const schoolBlocks = [...groups.entries()].map(([schoolName, school]) => {
    const shiftBlocks = [...school.shifts.entries()].map(([shiftName, classes]) => {
      const classBlocks = [...classes.entries()].map(([className, classExams]) => {
        const examCards = classExams.map(e => `
          <a class="exam-card" href="/provas/${e.id}">
            <div class="exam-card-top">
              <div>
                <span class="eyebrow">${e.trimester ? e.trimester + 'º trimestre' : 'Prova'}</span>
                <h4>${esc(e.title)}</h4>
              </div>
          ...
          <details class="class-group" open>
            <summary>
              ...
              <span>
                ...
                <small>${classExams.length} prova(s)</small>
              </span>
    ...
    return `
      <details class="school-group" open>
        <summary>
    ...
    nav() + `
      <main>
        <section class="welcome-panel">
          <div>
            <span class="eyebrow blue">PAINEL DO PROFESSOR</span>
            <h1>Olá, Professor Josemar!</h1>
            <p>Suas provas ficam organizadas por <b>escola, turno e turma</b>.</p>
          </div>
          <a class="btn create-btn" href="/provas/nova"><span>＋</span> Criar nova prova</a>
        </section>
        ...
        <div class="cards dashboard-stats">
          ...
          <div class="card stat stat-green"><span class="stat-icon">✅</span><b>${s.c}</b><span>Respostas recebidas</span></div>
          <div class="card stat stat-orange"><span class="stat-icon">🏫</span><b>${schoolCount.c || 0}</b><span>Escolas com provas</span></div>
        </div>

        <div class="section-heading">
          <div>
            <span class="eyebrow blue">ORGANIZAÇÃO</span>
            <h2>Provas por escola</h2>
            <p>Abra a escola, depois o turno e a turma para encontrar cada avaliação.</p>
          </div>
        </div>
        ...
        <div class="school-list">
          ${schoolBlocks || `
            <div class="card empty-state">
              ...
              <p>Quando você criar uma avaliação, ela aparecerá aqui dentro da escola e da turma corretas.</p>
  ...
  return `<style>
    ...
    *{box-sizing:border-box}
    ...
    body{
      ...
      background:
        radial-gradient(circle at top right,rgba(33,108,243,.08),transparent 30%),
        var(--bg);
      color:var(--ink);
    }
    ...
    nav{
      ...
      top:0;
      z-index:50;
      background:linear-gradient(135deg,#10233c,#17385f);
      padding:15px 20px;
      display:flex;
      justify-content:space-between;
      align-items:center;
      gap:12px;
      box-shadow:0 6px 18px rgba(16,35,60,.18);
    }
    nav a{color:white;margin-left:14px}
    .logo{font-weight:900;font-size:20px;margin:0;letter-spacing:-.3px}
    .logo span,h1 span{color:#55d68f}
    main{max-width:1100px;margin:28px auto;padding:0 16px}

    .hero,.results-header,.welcome-panel{
      display:flex;
      justify-content:space-between;
      gap:18px;
      align-items:center;
    ...
    .welcome-panel h1,.results-header h1{margin:5px 0 6px;font-size:clamp(27px,4vw,40px);letter-spacing:-1px}
    .welcome-panel p,.results-header p,.hero p{margin:0;color:var(--muted)}
    ...
    .card{
      ...
      padding:19px;
      margin-bottom:15px;
      box-shadow:0 6px 20px rgba(30,55,90,.06);
    }
    .stat{
      position:relative;
      overflow:hidden;
    ...
    .stat b{font-size:34px;line-height:1;margin-top:12px}
    .stat>span:last-child,small{color:var(--muted)}
    .stat-icon{font-size:24px}
    .stat-blue{color:#216cf3}
    .stat-purple{color:#7357df}
    .stat-green{color:#1eab65}
    .stat-orange{color:#e88c31}
    ...
    .btn,button{
      ...
      font-size:15px;
      cursor:pointer;
      box-shadow:0 6px 15px rgba(33,108,243,.16);
    }
    .btn:hover,button:hover{filter:brightness(.98)}
    .create-btn{padding:14px 19px}
    .secondary{
    ...
    input:focus,select:focus,textarea:focus{
      border-color:#7aa9ff;
      box-shadow:0 0 0 3px rgba(33,108,243,.10);
    }
    ...
    .pick input,.answer input{width:auto;margin-top:4px;margin-right:10px}
    .answer:hover{border-color:#a9c7ff;background:#f8fbff}
    ...
    .login{
      ...
      padding:30px;
      border-radius:22px;
      box-shadow:0 18px 55px rgba(25,45,80,.16);
      text-align:center;
    }
    .login form{display:grid;gap:12px;text-align:left;margin-top:20px}
    .alert{background:#fff0f0;color:#a42b2b;padding:11px;border-radius:10px;margin:12px 0}
    .success{
      width:64px;height:64px;border-radius:50%;
      background:#e6f8ee;color:var(--green);
      display:grid;place-items:center;margin:auto;
      font-size:35px;font-weight:900;
    }
    .grade{
      margin:18px 0;background:#f6f8fb;border-radius:15px;padding:19px;
      display:flex;flex-direction:column
    }
    .grade b{font-size:54px}

    .student{max-width:780px}
    .student-head{display:flex;gap:12px;align-items:center}
    .student-head.modern{
      background:white;border:1px solid var(--line);
      border-radius:20px;padding:20px;margin-bottom:16px;box-shadow:var(--shadow)
    }
    .student-head h1{margin:3px 0 4px}
    .student-head p{margin:0;color:var(--muted)}
    .logo-box{
      min-width:54px;width:54px;height:54px;border-radius:15px;
      background:linear-gradient(135deg,var(--blue),var(--green));
      display:grid;place-items:center;color:white;font-weight:900;
      box-shadow:0 8px 18px rgba(33,108,243,.22)
    }
    .student-id-card{display:grid;grid-template-columns:1.4fr 1fr;gap:14px;align-items:end}
    .fixed-class{
      background:#f5f8fd;border:1px solid var(--line);border-radius:12px;padding:12px;
      display:flex;flex-direction:column;gap:3px
    }
    .fixed-class span,.fixed-class small{font-size:12px;color:var(--muted)}
    .fixed-class b{font-size:22px;color:var(--blue)}
    .question-card h2{font-size:19px;line-height:1.4}
    .qnum{font-size:12px;color:var(--blue);font-weight:900;text-transform:uppercase;letter-spacing:.6px}
    ...
    .bar{height:12px;background:#edf1f6;border-radius:12px;overflow:hidden}
    .bar i{display:block;height:100%;background:linear-gradient(90deg,var(--blue),var(--green))}
    .bar.good i{background:linear-gradient(90deg,#22a965,#53cc89)}
    .bar.mid i{background:linear-gradient(90deg,#e6a22d,#f2c760)}
    .bar.low i{background:linear-gradient(90deg,#df5a5a,#ee8585)}
    .pct-badge{padding:4px 8px;border-radius:999px;font-size:12px}
    .pct-badge.good{background:#e8f8ef;color:#16814a}
    .pct-badge.mid{background:#fff5dc;color:#9b6812}
    .pct-badge.low{background:#ffeded;color:#a23a3a}
    .performance-card{
      padding:14px;border:1px solid var(--line);border-radius:14px;background:#fbfcfe
    }
    .performance-card small{display:block;margin-top:8px;line-height:1.4}
    .student-number{
      display:inline-grid;place-items:center;
      width:25px;height:25px;border-radius:50%;
      background:#edf4ff;color:var(--blue);font-size:11px;font-weight:800;margin-right:5px
    }

    .section-heading{margin:28px 2px 14px}
    .section-heading h2{margin:4px 0;font-size:25px}
    .section-heading p{margin:0;color:var(--muted)}
    .school-list{display:grid;gap:15px}
    ...
    .school-group>summary{
      display:flex;justify-content:space-between;align-items:center;
      gap:14px;padding:18px;cursor:pointer;
      background:linear-gradient(135deg,#fff,#f7faff)
    }
    .school-summary-main{display:flex;align-items:center;gap:13px;min-width:0}
    .school-icon{
      width:48px;height:48px;border-radius:14px;
      background:#eaf2ff;display:grid;place-items:center;font-size:24px;flex:none
    }
    .school-group h2{margin:0 0 4px;font-size:18px}
    .school-group p{margin:0;color:var(--muted);font-size:13px}
    .summary-chevron{font-size:28px;color:#8aa0bd;transition:.2s}
    details[open]>summary .summary-chevron{transform:rotate(90deg)}
    .school-content{padding:0 17px 17px}
    .shift-block{border-top:1px solid var(--line);padding-top:15px;margin-top:2px}
    .shift-title{display:flex;align-items:center;gap:8px;margin-bottom:10px}
    .shift-title h3{margin:0;font-size:15px}
    .shift-dot{width:9px;height:9px;border-radius:50%;background:var(--blue)}
    .class-group{
      border:1px solid var(--line);border-radius:14px;margin:9px 0;background:#fbfcfe;overflow:hidden
    }
    .class-group>summary{
      cursor:pointer;padding:12px 13px;
      display:flex;align-items:center;gap:10px
    }
    .class-group>summary>span:nth-child(2){display:flex;flex-direction:column;gap:2px;flex:1}
    .class-icon{font-size:19px}
    .exam-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;padding:0 12px 12px}
    .exam-card{
      display:block;border:1px solid var(--line);border-radius:13px;padding:13px;
      background:white;color:var(--ink);transition:.15s
    }
    .exam-card:hover{transform:translateY(-1px);border-color:#b8d0fb;box-shadow:0 8px 18px rgba(33,108,243,.08)}
    .exam-card-top{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}
    .exam-card h4{margin:4px 0 9px;font-size:15px}
    .status-pill{white-space:nowrap;padding:5px 8px;border-radius:999px;font-size:11px;font-weight:800}
    .status-pill.open{background:#e8f8ef;color:#16814a}
    .status-pill.closed{background:#eef1f5;color:#667386}
    .exam-metrics{display:flex;gap:10px;flex-wrap:wrap;color:var(--muted);font-size:12px}
    .exam-metrics b{color:var(--ink)}
    .empty-state{text-align:center;padding:35px}
    .empty-icon{font-size:40px}
    .card-heading-row{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:8px}
    .card-heading-row h2{margin:3px 0}
    .legend{display:flex;align-items:center;gap:5px;font-size:11px;color:var(--muted);flex-wrap:wrap}
    .legend i{width:8px;height:8px;border-radius:50%;display:inline-block;margin-left:5px}
    .legend-good{background:#34b874}.legend-mid{background:#e5aa37}.legend-low{background:#df6262}

    .mode-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
    .mode-card{
      display:flex;flex-direction:row;align-items:flex-start;gap:10px;
      border:1px solid var(--line);border-radius:13px;padding:14px;cursor:pointer;background:#fff
    }
    .mode-card input{width:auto;margin-top:4px}
    .info-box,.filter-note{
      margin-top:12px;padding:12px;border-radius:11px;background:#f0f6ff;color:#41516b
    }

    @media(max-width:760px){
      nav{align-items:flex-start}
      nav div{display:grid;grid-template-columns:1fr 1fr;gap:7px}
      nav a{margin:0}
      .hero,.results-header,.welcome-panel{align-items:flex-start;flex-direction:column}
      .cards{grid-template-columns:1fr 1fr}
      .grid{grid-template-columns:1fr}
      .span{grid-column:auto}
      .exam-grid{grid-template-columns:1fr}
      .student-id-card{grid-template-columns:1fr}
      .card-heading-row{align-items:flex-start;flex-direction:column}
    }
    @media(max-width:440px){
      main{padding:0 12px}
      .cards{grid-template-columns:1fr 1fr}
      .dashboard-stats .stat{min-height:122px;padding:15px}
      .dashboard-stats .stat b{font-size:29px}
      .school-group>summary{padding:14px}
      .school-icon{width:42px;height:42px}
      .school-content{padding:0 12px 12px}
