...
async function publicExam(request, env, token) {
  ...
  // Se o tempo terminou enquanto o aluno estava fora da página, finaliza com o que foi salvo.
  if (attempt && attempt.status === 'in_progress' && now >= Number(attempt.deadline_at)) {
    const result = await finalizeAttempt(env, e, qs.results, attempt, Number(attempt.deadline_at));
    return page('Resultado', studentResultPage(e, attempt.student_name, result, qs.results.length, true));
  }

  if (request.method === 'GET') {
    if (!attempt) return page(e.title, studentStartPage(e));
    if (attempt.status === 'submitted') {
      const result = await loadAttemptResult(env, attempt, qs.results.length);
      return page('Resultado', studentResultPage(e, attempt.student_name, result, qs.results.length, false));
    }
    ...
    const savedMap = Object.fromEntries(saved.results.map(x => [String(x.question_id), String(x.answer || '')]));
    return page(e.title, studentForm(e, qs.results, attempt, savedMap));
  }
  ...
  if (action === 'start') {
    if (attempt && attempt.status === 'in_progress') return redirect(`/p/${token}`);
    ...
    // Se o aluno voltar por outro navegador/dispositivo e digitar o mesmo nome completo,
    // recupera a tentativa em andamento em vez de reiniciar os 50 minutos.
    ...
    if (previousAttempt) {
      if (Math.floor(Date.now() / 1000) >= Number(previousAttempt.deadline_at)) {
        await finalizeAttempt(env, e, qs.results, previousAttempt, Number(previousAttempt.deadline_at));
      }
      return new Response(null, {
        status: 302,
        headers: {
          Location: `/p/${token}`,
          'Set-Cookie': `${cookieName}=${previousAttempt.attempt_token}; Path=/p/${token}; HttpOnly; Secure; SameSite=Lax; Max-Age=86400`
        }
      });
    }

    const attemptToken = crypto.randomUUID().replaceAll('-', '') + crypto.randomUUID().replaceAll('-', '').slice(0, 12);
    ...
    return new Response(null, {
      ...
      headers: {
        Location: `/p/${token}`,
        'Set-Cookie': `${cookieName}=${attemptToken}; Path=/p/${token}; HttpOnly; Secure; SameSite=Lax; Max-Age=86400`
      }
    });
  }
  ...
  if (!attempt) {
    ...
    return page(e.title, studentStartPage(e, 'Sua sessão não foi encontrada. Inicie a prova novamente.'), 409);
  }

  if (attempt.status === 'submitted') {
    if (action === 'autosave' || action === 'event') return jsonResponse({ ok: false, submitted: true }, 409);
    const result = await loadAttemptResult(env, attempt, qs.results.length);
    return page('Resultado', studentResultPage(e, attempt.student_name, result, qs.results.length, false));
  }
  ...
  if (current >= Number(attempt.deadline_at)) {
    const result = await finalizeAttempt(env, e, qs.results, attempt, Number(attempt.deadline_at));
    if (action === 'autosave' || action === 'event') {
      return jsonResponse({ ok: false, expired: true, redirect: `/p/${token}` }, 409);
    }
    return page('Resultado', studentResultPage(e, attempt.student_name, result, qs.results.length, true));
  }
  ...
  if (action === 'autosave') {
    ...
    const validQuestion = qs.results.some(q => Number(q.id) === qid);
  ...
  // Salva também as respostas presentes no formulário final para evitar perda em caso de falha de autosave.
  const finalSave = [];
  ...
  const result = await finalizeAttempt(env, e, qs.results, attempt, current);
  return page('Resultado', studentResultPage(e, attempt.student_name, result, qs.results.length, false));
}
      ...
      <main>
        ...
        <div class="card">
          ...
          <div class="table">
            <table>
              <thead>
                <tr><th>Aluno</th><th>Turma</th><th>Nota</th><th>Acertos</th><th>Tempo</th><th>Saídas da tela</th></tr>
              </thead>
