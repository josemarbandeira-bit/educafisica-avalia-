...
async function dashboard(env) {
  ...
  const rows = exams.results.map(e => `
    <tr>
      <td>
        <b>${esc(e.title)}</b><br>
        <small>
          ${esc(e.school_name || 'Sem escola')}
          ${e.shift ? ' · ' + esc(e.shift) : ''}
          ${e.class_name ? ' · ' + esc(e.class_name) : ''}
          ${e.trimester ? ' · ' + e.trimester + 'º tri' : ''}
        </small>
      </td>
      <td>${e.submissions}</td>
      <td>${e.avg_score ?? '—'}</td>
      <td><a class="btn small" href="/provas/${e.id}">Abrir</a></td>
    </tr>
  `).join('');
      ...
      <main>
        <div class="hero">
          <div>
            ...
            <p>Provas online, correção automática e gráficos.</p>
          </div>
          <a class="btn" href="/provas/nova">+ Criar Prova</a>
        </div>

        <div class="cards">
          <div class="card stat"><b>${q.c}</b><span>Questões</span></div>
          <div class="card stat"><b>${exams.results.length}</b><span>Provas</span></div>
          <div class="card stat"><b>${s.c}</b><span>Respostas</span></div>
        </div>
        ...
        <div class="card">
          ...
          <div class="table">
            <table>
              <thead>
                <tr><th>Prova</th><th>Respostas</th><th>Média</th><th></th></tr>
              </thead>
              <tbody>${rows || '<tr><td colspan="4">Nenhuma prova criada.</td></tr>'}</tbody>
            </table>
          </div>
        </div>
      </main>
...
async function newExam(env) {
  ...
  const classes = await env.DB.prepare(`
    SELECT c.id,c.school_id,c.shift,c.class_name,c.grade,s.name AS school_name
    FROM school_classes c
    JOIN schools s ON s.id=c.school_id
    WHERE c.active=1
    ORDER BY s.name,c.shift,c.grade,c.class_name
  `).all();
  ...
  const classOptions = classes.results.map(c =>
    `<option
      value="${c.id}"
      data-school="${c.school_id}"
      data-shift="${attr(c.shift)}"
      data-grade="${attr(c.grade)}"
    >${esc(c.class_name)} · ${esc(c.grade)} · ${esc(c.shift)}</option>`
  ).join('');
    ...
    nav() + `
      <main>
        <div class="hero">
          <div>
            <h1>Criar Prova</h1>
            <p>Escolha a turma e gere a prova automaticamente ou selecione as questões.</p>
          </div>
          <a class="btn secondary" href="/">← Painel</a>
        </div>
        ...
        <form method="post" id="examCreateForm">
          <div class="card grid">
            ...
            <label>
              Turma
              <select name="school_class_id" id="classSelect" required>
                <option value="">Selecione escola e turno</option>
                ${classOptions}
              </select>
            </label>
          ...
          <div class="card" id="autoBox">
            ...
            <div class="info-box">
              O sistema usa a série da turma escolhida e considera questões do trimestre selecionado
              ou questões marcadas como <b>Geral</b>.
            </div>
          ...
          <div class="card" id="manualBox" style="display:none">
            ...
            <div class="filter-note">
              Você pode marcar qualquer questão abaixo.
            </div>
            ...
            <button style="margin-top:15px">
              Criar com questões selecionadas
            </button>
          </div>
        </form>
      </main>
      ...
      <script>
        ...
        const allOptions = [...classSelect.querySelectorAll('option[data-school]')];
        ...
        function filterClasses() {
          ...
          allOptions
            .filter(o =>
              (!s || o.dataset.school === s) &&
              (!t || o.dataset.shift === t)
            )
            .forEach(o => classSelect.appendChild(o.cloneNode(true)));
        }

        school.addEventListener('change', filterClasses);
        shift.addEventListener('change', filterClasses);
        filterClasses();
...
async function createExam(request, env) {
  ...
  const classId = Number(f.get('school_class_id'));
  ...
  if (!classInfo) {
    return page(
      'Atenção',
      '<div class="login"><h2>Turma inválida. Volte e selecione novamente.</h2><a class="btn" href="/provas/nova">Voltar</a></div>',
      400
    );
  }
  ...
  if (mode === 'manual') {
    ...
    if (ids.length < count) {
      const criteria = [
        classInfo.grade || 'série não definida',
  ...
  `).bind(
    token,
    f.get('title'),
    classInfo.grade && classInfo.grade.includes('série') ? 'Médio' : 'Fundamental',
    classInfo.grade || '',
